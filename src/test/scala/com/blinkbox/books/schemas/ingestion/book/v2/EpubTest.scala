...
import com.blinkbox.books.schemas.ingestion.book.v2.Epub.Ingestion
import org.scalatest.FunSuite

class EpubTest extends FunSuite {
  ...
  test("Construct and destructure Epub Ingested message") {
    val event = Event.apply(EventHeader("epub"), EventBody(epubIngestionBytes, ContentType(Epub.jsonMediaType, None)))
    val msg = Epub.convert[Ingestion](event)
